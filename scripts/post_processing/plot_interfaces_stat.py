...
def main():
    ...
    data = {
        ...
        name: {"t": [], "fnx": [], "fny": [], "fnz": [], "ftx": [], "fty": [], "ftz": [], "posx": [], "posy": [], "posz": [], 'type': [], 'dn': []}
    ...
    for inter_file in interaction_files:
        ...
        for c in contacts:
            ...
            data[name]["dn"].append(c.dn)

        # Contacts absents à ce pas de temps
        for name in tracked.values():
            if name not in found_contacts:
                data[name]["t"].append(time)
                for key in ["fnx","fny","fnz","ftx","fty","ftz","posx","posy","posz"]:
                    data[name][key].append(float("nan"))
                data[name]["type"].append(-1)
                data[name]["dn"].append(float("nan"))
    ...
    with open("debug_contacts.txt", "w") as f:
        ...
        for name in data:
            header += f" {name}_fnx {name}_fny {name}_fnz {name}_ftx {name}_fty {name}_ftz {name}_posx {name}_posy {name}_posz {name}_type {name}_dn"
        f.write(header + "\n")

        #n = min(len(global_stats["t"]),
        #    min(len(data[name]["t"]) for name in data))
        n = len(global_stats["t"])
        ...
        for i in range(n):
            ...
            for name in data:
                line += f" {data[name]['fnx'][i]} {data[name]['fny'][i]} {data[name]['fnz'][i]} {data[name]['ftx'][i]} {data[name]['fty'][i]} {data[name]['ftz'][i]} {data[name]['posx'][i]} {data[name]['posy'][i]} {data[name]['posz'][i]} {data[name]['type'][i]} {data[name]['dn'][i]}"
