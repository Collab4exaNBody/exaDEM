...
inline double compute_damp(const double damp_rate, const double kn, const double meff) {
  const double ret = damp_rate * 2.0 * sqrt(kn * meff);
  return ret;
}

ONIKA_HOST_DEVICE_FUNC
inline Vec3d compute_relative_velocity(const Vec3d& contact_position, const Vec3d& pos_i, const Vec3d& vel_i,
                                       const Vec3d& vrot_i, const Vec3d& pos_j, const Vec3d& vel_j,
                                       const Vec3d& vrot_j) {
...
ONIKA_HOST_DEVICE_FUNC inline Vec3d compute_normal_force(const double kn, const double fc, const double damp,
                                                         const double dn, const double vn, const Vec3d& n);

template <>
ONIKA_HOST_DEVICE_FUNC inline Vec3d compute_normal_force<false>(const double kn, const double fc, const double damp,
                                                                const double dn, const double vn, const Vec3d& n) {
  return normal_force(kn, damp, dn, vn) * n;
}

template <>
ONIKA_HOST_DEVICE_FUNC inline Vec3d compute_normal_force<true>(const double kn, const double fc, const double damp,
                                                               const double dn, const double vn, const Vec3d& n) {
  const double fn_norm = normal_force(kn, damp, dn, vn) - fc;
...
ONIKA_HOST_DEVICE_FUNC
inline Vec3d compute_tangential_force(const double dt, const double vn, const Vec3d& n, const Vec3d& vel) {
...
ONIKA_HOST_DEVICE_FUNC
inline Vec3d compute_tangential_force(const double kt, const double dt, const double vn, const Vec3d& n,
                                      const Vec3d& vel) {
...
ONIKA_HOST_DEVICE_FUNC
inline double compute_effective_radius(const double ri, const double rj) {
...
//
ONIKA_HOST_DEVICE_FUNC
inline double compute_normal_force_value_with_cohesive_force(const double kn, const double fc, const double damp,
                                                             const double dn, const double vn) {
  const double fne = -kn * dn;   // elastic contact
  const double fnv = damp * vn;  // viscous damping
  const double fn = fnv + fne - fc;
  const double ret = fn < (-fc) ? -fc : fn;
  return ret;
}

ONIKA_HOST_DEVICE_FUNC
inline double compute_threshold_ft(const double mu, const double kn, const double dn) {
  // === recompute fne
...
ONIKA_HOST_DEVICE_FUNC
inline double compute_threshold_ft_with_cohesive_force(const double mu, const double fn, const double fc) {
  const double threshold_ft = std::fabs(mu * (fn + fc));
  return threshold_ft;
}

ONIKA_HOST_DEVICE_FUNC
inline void fit_tangential_force(const double threshold_ft, Vec3d& ft) {
