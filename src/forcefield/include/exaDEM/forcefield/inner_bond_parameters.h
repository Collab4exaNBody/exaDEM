...
namespace exaDEM
{
  ...
  struct InnerBondParams
  {
    ...
    double mu = 0.0;         /**< Friction coefficient (Coulomb friction model). */
    double en = 0.0;
    double pow = 0.0;
    double g  = 0.0;
  };
   ...
   */
  template<> inline void display_header<InnerBondParams>()
  {
    lout << "=======================================================================================================" << std::endl;
    lout << "|        typeA |        typeB |        kn |        kt |        mu |        en |       pow |         g |" << std::endl;
    lout << "-------------------------------------------------------------------------------------------------------" << std::endl;
  }
   ...
   */
  template<> inline void display_end_table<InnerBondParams>()
  {
    lout << "=======================================================================================================" << std::endl;
  }
  ...
  {
    std::string line = onika::format_string(" %.3e | %.3e | %.3e | %.3e | %.3e | %.3e |", 
        params.kn, 
        params.kt, 
        params.mu, 
        params.en, 
...
namespace YAML
{
  ...
  template <> struct convert<InnerBondParams>
  {
    static bool decode(const Node &node, InnerBondParams &v)
    {
      ...
      v.en = node["en"].as<Quantity>().convert();
      v.pow = node["pow"].as<Quantity>().convert();
