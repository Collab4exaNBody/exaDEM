...
class UpdatePersistentInteractionsOperator : public OperatorNode {
 ...
 public:
  ...
  inline std::tuple<bool, uint16_t> locate_particle_id(const uint64_t* ids, size_t n, uint64_t id) {
          ...
          //      else
          {
            ...
            if (!find) {
              color_log::error("update_persistent_interactions",
                               "The particle with the particle id " + std::to_string(id) +
                                   " is not located in the cell " + std::to_string(cell_a));
            }
          }
