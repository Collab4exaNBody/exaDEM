...
class ComputeInnerBondForce : public OperatorNode {
 ...
 public:
  ...
  void scan_kn() {
    ...
    double kn = 0.0;
    auto get_max_kn = [&kn] (const InnerBondParams& cp) -> void {
      kn = std::max(kn, cp.kn);
    };
...
#     define __params__ cells, vertex_fields, ibp, shps, time
    ...
    const MultiMatContactParamsTAccessor<InnerBondParams> ibp =
        force_law_parameters.get_multimat_accessor();

    if (is_def_xform) {
      inner_bond_law<LinearXForm> func;
      func.xform = LinearXForm{xform};
      run_contact_law<InteractionTypeId::InnerBond>(
          parallel_execution_context(), classifier, func, __params__);
    } else {
