...
namespace exaDEM
{
  ...
  {
    ParticleTypeInt* __restrict__ ptype;
    double* __restrict__ prx;
    double* __restrict__ pry;
    ...
    VerticesType* __restrict__ pvtx;
		...
		template<typename Cells>
			ONIKA_HOST_DEVICE_FUNC inline cell_accessors(Cells& cell)
			{
				ptype =  cell[field::type];
				prx   =  cell[field::rx];
				pry   =  cell[field::ry];
				prz   =  cell[field::rz];
				pid   =  cell[field::id];
	...
	ONIKA_HOST_DEVICE_FUNC bool intersect(double rVerlet, OBB& obb_i, particle_info& p_nbh)
	{
