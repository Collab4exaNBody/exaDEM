...
class ReadShapeFileOperator : public OperatorNode {
  ...
  ADD_SLOT(std::vector<double>, scale_factor, INPUT, OPTIONAL,
           DocString{"This option 'scale_factor' the input shapes. OBB, volume, vertices, and intertia are recomputed. "
                     "Note that a vector of double should be provided. Example: scale_factor: [1.2,1,5.2]"});
  ...
  inline void execute() override final {
    ...
    if (rename.has_value()) {
      ...
      if (list_of_shapes.size() != names.size()) {
        color_log::error("read_shape_file", "The vector size 'rename' should have " +
                                                std::to_string(list_of_shapes.size()) + " + elements and not " +
                                                std::to_string(names.size()) + "elements");
      }
    ...
    if (scale_factor.has_value()) {
      ...
      if (list_of_shapes.size() != scales.size()) {
        color_log::error("read_shape_file", "The vector size 'scale_factor' should have " +
                                                std::to_string(list_of_shapes.size()) + " + elements and not " +
...
// === register factories ===
ONIKA_AUTORUN_INIT(read_shape_file) {
