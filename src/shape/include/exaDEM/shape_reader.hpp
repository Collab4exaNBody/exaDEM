  ...
  while (1) {
    ...
    if (key == "preCompDone") {
      ...
      if (preCompDoneType != "y") {
        color_log::error("read_shp",
                         "The “preComnDone” key should be set to “y.” Verify that quantities such as I/m and volume "
 ...
 */
inline std::vector<shape> read_shps(const std::string file_name, bool big_shape = false, bool vtk = false) {
 ...
 */
inline shape read_shp(const std::string file_name, bool big_shape = false) {
  std::ifstream input(file_name.c_str());
  for (std::string line; getline(input, line);) {
    if (line == "<") {
