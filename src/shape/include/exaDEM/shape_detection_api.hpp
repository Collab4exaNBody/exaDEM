...
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_vertex(
    const double rVerlet, const Vec3d& pi, const double radius, const Vec3d& pj,
    const int j, const double hj, const shape* shpj,
    const exanb::Quaternion& oj) {
  // === compute vertex position
  Vec3d vj = shpj->get_vertex(j, pj, hj, oj);
  return filter_vertex_vertex(rVerlet,
                              pi, radius,
                              vj, shpj->minskowski(hj));
}

ONIKA_HOST_DEVICE_FUNC
inline bool filter_vertex_vertex(const double rVerlet, const Vec3d& pi,
  ...
  Vec3d vj = shpj->get_vertex(j, pj, hj, oj);
  return filter_vertex_vertex(rVerlet, vi, shpi->minskowski(hi), vj,
                              shpj->minskowski(hj));
}

template <typename VecI, typename VecJ>
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_vertex(
    const double rVerlet, VecI& vai, const double hi, const int i,
  ...
  // === compute vertex position
  return filter_vertex_vertex(rVerlet, vai[i], shpi->minskowski(hi), vaj[j],
                              shpj->minskowski(hj));
}
...
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_vertex_v2(
    ...
    const Vec3d& pi, const double ri,
...
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_vertex_v2(
    const double rVerlet,
    const VecI& vai, const double hi, const int i, const shape* shpi,
    const VecJ& vaj, const double hj, const int j, const shape* shpj) {
  assert(i < shpi->get_number_of_vertices());
  assert(j < shpj->get_number_of_vertices());

  // === compute vertex position
  return filter_vertex_vertex(rVerlet,
                              vai[i], shpi->minskowski(hi),
                              vaj[j], shpj->minskowski(hj));
}

ONIKA_HOST_DEVICE_FUNC inline contact detection_vertex_vertex(
    const Vec3d& pi, const double hi, const int i, const shape* shpi,
    const exanb::Quaternion& oi, const Vec3d& pj, const double hj, const int j,
    const shape* shpj, const exanb::Quaternion& oj) {
  // === compute vertex position
  Vec3d vi = shpi->get_vertex(i, pi, hi, oi);
  Vec3d vj = shpj->get_vertex(j, pj, hj, oj);
  return detection_vertex_vertex_core(vi, shpi->minskowski(hi),
                                      vj, shpj->minskowski(hj));
}

ONIKA_HOST_DEVICE_FUNC inline contact detection_vertex_vertex(
    const Vec3d& pi, const double radius, const Vec3d& pj, const double hj,
    const int j, const shape* shpj, const exanb::Quaternion& oj) {
  // === compute vertex position
  Vec3d vj = shpj->get_vertex(j, pj, hj, oj);
  return detection_vertex_vertex_core(pi, radius,
                                      vj, shpj->minskowski(hj));
}
...
ONIKA_HOST_DEVICE_FUNC
inline bool filter_vertex_edge(const double rVerlet, const Vec3d& pi,
                               const double hi, const int i, const shape* shpi,
                               const exanb::Quaternion& oi, const Vec3d& pj,
...
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_edge(
    const double rVerlet, const Vec3d& pi, const double radius,
    const Vec3d& pj, const double hj, const int j,
    const shape* shpj, const exanb::Quaternion& oj) {
  // === compute vertice positions
  auto [first, second] = shpj->get_edge(j);
...
template <typename VecJ>
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_edge(const double rVerlet,
                                                      const Vec3d& vi,
                                                      const double ri,
                                                      VecJ& vaj,
                                                      const double hj,
...
ONIKA_HOST_DEVICE_FUNC inline bool filter_vertex_edge(
  ...
  double ri = shpi->minskowski(hi);
  double rj = shpj->minskowski(hj);
  return filter_vertex_edge_core(rVerlet,
                                 vai[i], ri,
                                 vaj[first], vaj[second], rj);
}
...
template <typename VecI, typename VecJ>
ONIKA_HOST_DEVICE_FUNC inline contact detection_vertex_edge(
    VecI& vai, const double hi, const int i, const shape* shpi,
...
template <typename VecJ>
ONIKA_HOST_DEVICE_FUNC inline contact detection_vertex_face(const Vec3d& vi,
                                                            const double ri,
                                                            VecJ& vaj,
                                                            const double hj,
                                                            const int j,
...
ONIKA_HOST_DEVICE_FUNC inline bool filter_edge_edge(
  ...
  return filter_edge_edge_core(rVerlet,
                               vai[fi], vai[si], ri,
                               vaj[fj], vaj[sj], rj);
}

ONIKA_HOST_DEVICE_FUNC
inline bool filter_edge_edge(const double rVerlet,
                             const Vec3d& pi, const double hi, const int i,
                             const shape* shpi, const exanb::Quaternion& oi,
                             const Vec3d& pj, const double hj, const int j,
                             const shape* shpj, const exanb::Quaternion& oj) {
...
// API edge - edge
ONIKA_HOST_DEVICE_FUNC
inline contact detection_edge_edge(const Vec3d& pi, const double hi, const int i,
                                   const shape* shpi, const exanb::Quaternion& oi,
                                   const Vec3d& pj, const double hj, const int j,
                                   const shape* shpj, const exanb::Quaternion& oj) {
  double ri = shpi->minskowski(hi);
...
#define __params__                                       \
    const VertexType &vai, const double hi, const int i, const shape *shpi, \
...
#define __params__                                                       \
    const VertexType &pi, const double hi, const int i, const shape *shpi,\
    const Vec3d *pj, const double hj, const int j, const shape *shpj
#define __params__sph__                                           \
    const Vec3d &pi, const double ri, const Vec3d &pj, const double hj,\
    const int j, const shape *shpj, const exanb::Quaternion &oj
