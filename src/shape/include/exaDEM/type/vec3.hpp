...
class vec3 {
 public:
  ...
  ONIKA_HOST_DEVICE_FUNC inline bool isnull(const T tol = 1e-20) const {
    return (fabs(x) < tol && fabs(y) < tol && fabs(z) < tol);
  }

  ONIKA_HOST_DEVICE_FUNC inline T* c_vec() {
    return &x;
  }

  ONIKA_HOST_DEVICE_FUNC inline T& operator[](int i) {
    return (&x)[i];
  }
  ...
  ONIKA_HOST_DEVICE_FUNC inline const T n() const {
    return x;
  }

  ONIKA_HOST_DEVICE_FUNC inline const T t() const {
    return y;
  }

  ONIKA_HOST_DEVICE_FUNC inline const T s() const {
    return z;
  }
  ...
  ONIKA_HOST_DEVICE_FUNC inline friend vec3 operator+(const vec3& a, const vec3& b) {
  ...
  /// Dot product
  ONIKA_HOST_DEVICE_FUNC inline friend T operator*(const vec3& a, const vec3& b) {
    return (a.x * b.x + a.y * b.y + a.z * b.z);
  }

  ONIKA_HOST_DEVICE_FUNC inline friend T dot(const vec3& a, const vec3& b) {
    return (a.x * b.x + a.y * b.y + a.z * b.z);
  }
  ...
  ONIKA_HOST_DEVICE_FUNC inline friend vec3<T> component_max(const vec3<T>& a, const vec3<T>& b) {
    return vec3<T>((a.x > b.x) ? a.x : b.x, (a.y > b.y) ? a.y : b.y, (a.z > b.z) ? a.z : b.z);
  }

  /// Absolut value of the components
  ONIKA_HOST_DEVICE_FUNC inline friend vec3<T> component_abs(const vec3<T>& a) {
    return vec3<T>(fabs(a.x), fabs(a.y), fabs(a.z));
  }

  /// Cross product
  ONIKA_HOST_DEVICE_FUNC inline friend vec3<T> operator^(const vec3<T>& a, const vec3<T>& b) {
...
#define NQ(X, Y, Z, N)                 \
  ...
  // Comparisons
  ONIKA_HOST_DEVICE_FUNC inline bool operator==(const vec3<T>& other) const {
