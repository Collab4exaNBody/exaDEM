  ...
  inline void execute() final {
    ...
    *lb_inbalance_max = 0.0;
    ...
    std::string line = onika::format_string(
        "%9ld % .6e %13ld  %c %c %8s %.3e    %9.3f  %9.3f % .3e % .3e % .3e % .3e % .4e",
        *timestep,                            // %9ld
        *physical_time,                       // %.6e
...
ONIKA_AUTORUN_INIT(print_simulation_state) {
  OperatorNodeFactory::instance()->register_factory("print_simulation_state",
                                                    make_simple_operator<PrintSimulationStateNode>);
}
