...
struct SimulationStateReducFunc {
  template<InteractionType IT, typename PEC>
	void operator()(ClassifierContainer<IT>& container, int coef,
                  const double* const dnp, 
                  IOSimInteractionResult& result,
                  PEC& pec) {
		...
		if (size > 0 && dnp != nullptr) {  // skip it if forces has not been computed
		  reduce_data<IT, IOSimInteractionFunctor, IOSimInteractionResult>(
		    pec(), dataWrapper, func, size, result);
		}
	}
};
...
struct SimulationStateNode : public OperatorNode {
	...
	inline IOSimInteractionResult reduce_sim_io(Classifier& classifier, bool symetric) {
		...
		auto pec = [this] () { return this->parallel_execution_context(); };
		results.resize(types);
		{
      SimulationStateReducFunc func;
			for (int typeID = 0 ; typeID < types ; typeID++) {
        assert(typeID < classifier.buffers.size());
				const auto& buffs = classifier.buffers[typeID];
				const double* const dnp = onika::cuda::vector_data(buffs.dn);
				int coef = 1;
				if (typeID < InteractionTypeId::NTypesPP && symetric) {
          coef *= 2;
        }
        CDispatcher::dispatch(typeID, classifier, func, coef, dnp, results[typeID], pec);
			}
		}  // synchronize
