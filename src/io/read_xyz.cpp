...
class ReadXYZ : public OperatorNode {
  ...
  ADD_SLOT(ParticleTypes, particle_types,
           INPUT);  // optional. if no species given, type ids are allocated automatically
  ...
  inline void execute() final {
    ...
    if (rank == 0) {
      ...
      if (*adjust_bounds_to_particles) {
        ...
        file_bounds = {{min_x - (*enlarge_bounds), min_y - (*enlarge_bounds), min_z - (*enlarge_bounds)},
                       {max_x + (*enlarge_bounds), max_y + (*enlarge_bounds), max_z + (*enlarge_bounds)}};
...
__attribute__((constructor)) static void register_factories() {
  OperatorNodeFactory::instance()->register_factory("read_xyz", make_grid_variant_operator<ReadXYZ>);
}
