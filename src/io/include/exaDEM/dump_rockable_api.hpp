 ...
 */
template <typename STREAM>
Particle decrypt_particle(STREAM&& input, exanb::ParticleTypeMap& ptm) {
  std::string type_name;
  Particle p;
  input >> type_name;
  p.type = ptm[type_name];

  input >> p.group >> p.cluster >> p.homothety >> p.pos.x >> p.pos.y >> p.pos.z >> p.vel.x >> p.vel.y >> p.vel.z >>
      p.acc.x >> p.acc.y >> p.acc.z >> p.Q.w >> p.Q.x >> p.Q.y >> p.Q.z >> p.vrot.x >> p.vrot.y >> p.vrot.z >>
...
struct ConfReader {
  ...
  bool check() {
    ...
    if (int(particles.size()) != (n_particles - nDriven)) {
      return false;
    }
    // Check that interactions size is not larger than expected (drivers interactions may be excluded)
    if (int(interactions.size()) > n_interactions) {
      return false;  // remove interactions with drivers
  ...
  void read_stream(std::ifstream& file) {
      ...
      if (key == "t") {
        ...
        if (dt > 0.0) {
          color_log::warning("read_conf_rockable",
                             "'dt' is being overridden by the value defined in the rockable configuration file.");
        } else {
          color_log::error(
              "read_conf_rockable",
              "'dt' is <= 0.0. Please define a correct value for 'dt' in your rockable configuration file.");
        }
      } else if (key == "density") {
        int group;
        input >> group;
        if (group + 1 >= int(densities.size())) {
          densities.resize(group + 1);
        }
        ...
        if (max_warning_displayed > 0) {
          color_log::warning("read_conf_rockable", "The key '" + key + "' is skipped.");
        }
      }
    }
    ...
    if (max_warning_displayed < 0) {
      onika::lout << ansi::yellow(msg) << std::endl;
    } else {
      onika::lout << ansi::green(msg) << std::endl;
    }
  }
};
}  // namespace rockable
