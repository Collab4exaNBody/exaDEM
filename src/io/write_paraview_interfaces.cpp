...
namespace exaDEM
{
  ...
  template <class GridT, class = AssertGridHasFields<GridT>> class WriteParaviewInterfaceOperator : public OperatorNode
  {
    ...
    inline void execute() override final
    {
      ...
      for (size_t i=0; i<number_of_interfaces ; i++)
      {
				...
				for(size_t j=interface.loc ; j<interface.loc+interface.n ; j++)
				{
					...
					Vec3d v = shp->get_vertex(loc.sub, r, quat); 
	...
	template <class GridT> using WriteParaviewInterfaceOperatorTemplate = WriteParaviewInterfaceOperator<GridT>;

	// === register factories ===
	ONIKA_AUTORUN_INIT(write_paraview_interfaces) { OperatorNodeFactory::instance()->register_factory("write_paraview_interfaces", make_grid_variant_operator<WriteParaviewInterfaceOperatorTemplate>); }
} // namespace exaDEM
