...
template <>
constexpr DRIVER_TYPE get_type<exaDEM::Cylinder>() {
  return DRIVER_TYPE::CYLINDER;
}
template <>
constexpr DRIVER_TYPE get_type<exaDEM::Surface>() {
  return DRIVER_TYPE::SURFACE;
}
template <>
constexpr DRIVER_TYPE get_type<exaDEM::Ball>() {
  return DRIVER_TYPE::BALL;
}
template <>
...
struct Driver {
  ...
  virtual std::tuple<bool, double, exanb::Vec3d, exanb::Vec3d> detector(const double, const exanb::Vec3d&);
};
