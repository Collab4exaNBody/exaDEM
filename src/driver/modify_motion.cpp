...
namespace exaDEM
{
  ...
  class ModifyMotionBehavior : public OperatorNode
  {
    ...
    ADD_SLOT(Driver_params, motion, INPUT, REQUIRED, DocString{"List of params, motion type, motion vectors .... Example is { motion_type: STATIONARY}."});
    ...
    inline void execute() override final
    {
      ...
      if( std::abs(double(*time) - double(*physical_time)) > (*dt / 2) ) return;
      ...
      if( *display ) new_motion.print_driver_params();

      auto set_motion_type = [&new_motion] (auto& d) -> void { d.set_params(new_motion); };
      drvs.apply(*id, set_motion_type);
    }
  };

  // === register factories ===
  ONIKA_AUTORUN_INIT(modify_motion) { OperatorNodeFactory::instance()->register_factory("modify_motion", make_simple_operator<ModifyMotionBehavior>); }
} // namespace exaDEM
