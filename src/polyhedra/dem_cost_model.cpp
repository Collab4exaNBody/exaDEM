...
//	const size_t n_cells = grid_cell_count( dims );
	...
	if ( interactions.size() != n_cells ) interactions.resize( n_cells );
...
#     pragma omp parallel
  ...
  {
    OperatorNodeFactory::instance()->register_factory( "dem_cost_model", make_grid_variant_operator< DEMCostModelTmpl > );
  }
}
