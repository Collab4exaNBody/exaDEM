...
namespace exaDEM
{
	...
	template < typename GridT >
		struct NetworkFunctor
		{
			 ...
			 */
			static constexpr signature detection[4] ={
			...
			void operator () ( exaDEM::Interaction* I, const size_t offset, const size_t size)
			{
				...
				auto sort = [] (CoupleType& in) -> void { if( in.first > in.second) std::swap(in.first, in.second);};
