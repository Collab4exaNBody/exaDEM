...
template <bool multimat, ContactLawType ContactLaw, CohesiveLawType CohesiveLaw, typename GridT,
          class = AssertGridHasFields<GridT, field::_vx, field::_vy, field::_vz, field::_mom, field::_orient,
                                      field::_vrot, field::_radius>>
class ComputeContactClassifierSphere : public OperatorNode {
  ...
  ADD_SLOT(ContactParams, config, INPUT, OPTIONAL, DocString{"Contact parameters for sphere interactions"});
  ADD_SLOT(ContactParams, config_driver, INPUT, OPTIONAL, DocString{"Contact parameters for drivers, optional"});
 ...
 public:
  ...
  void core(XFormT& xform) {
    ...
    if (!multimat) {
      ...
      run_contact_law<InteractionTypeId::VertexCylinder>(parallel_execution_context(), classifier, cyl, cells, drvs,
                                                         cp_drvs, time);
      run_contact_law<InteractionTypeId::VertexSurface>(parallel_execution_context(), classifier, surf, cells, drvs,
                                                        cp_drvs, time);
  ...
  void check_slots() {
    ...
    /** polyhedron interactions are defined while the contact sphere operator is used */
    {
      ...
      for (int i = 1; i <= 3; i++) {
        ...
        if (size > 0) {
          std::string msg =
              "The contact operator for spheres is being used, but polyhedron interactions are defined.\n";
      ...
      if constexpr (!LawComboTraits<LawCombo>::cohesive) {
        if (config->dncut > 0) {
          color_log::error(opertor_name(),
                           "dncut is != 0 while the cohesive force is not used. Please, use "
                           "contact_[InputType]_[Shape]_hooke_cohesive operators.");
        }
...
using CCSMSHNoneTmpl = ComputeContactClassifierSphere<false, ContactLawType::Hooke, CohesiveLawType::None, GridT>;
template <class GridT>
using CCSMSHDmtTmpl = ComputeContactClassifierSphere<false, ContactLawType::Hooke, CohesiveLawType::DMT, GridT>;
template <class GridT>
using CCSMSHCohesiveTmpl =
    ComputeContactClassifierSphere<false, ContactLawType::Hooke, CohesiveLawType::Cohesive, GridT>;

template <class GridT>
using CCMMSHNoneTmpl = ComputeContactClassifierSphere<true, ContactLawType::Hooke, CohesiveLawType::None, GridT>;
template <class GridT>
using CCMMSHCohesiveTmpl =
    ComputeContactClassifierSphere<true, ContactLawType::Hooke, CohesiveLawType::Cohesive, GridT>;
template <class GridT>
using CCMMSHDmtTmpl = ComputeContactClassifierSphere<true, ContactLawType::Hooke, CohesiveLawType::DMT, GridT>;

// === register factories ===
ONIKA_AUTORUN_INIT(contact_force_sphere) {
  OperatorNodeFactory::instance()->register_factory("contact_sphere", make_grid_variant_operator<CCSMSHNoneTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_singlemat_sphere_hooke",
                                                    make_grid_variant_operator<CCSMSHNoneTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_singlemat_sphere_hooke_none",
                                                    make_grid_variant_operator<CCSMSHNoneTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_singlemat_sphere_hooke_dmt",
                                                    make_grid_variant_operator<CCSMSHDmtTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_singlemat_sphere_hooke_cohesive",
                                                    make_grid_variant_operator<CCSMSHCohesiveTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_multimat_sphere",
                                                    make_grid_variant_operator<CCMMSHNoneTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_multimat_sphere_hooke",
                                                    make_grid_variant_operator<CCMMSHNoneTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_multimat_sphere_hooke_none",
                                                    make_grid_variant_operator<CCMMSHNoneTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_multimat_sphere_hooke_cohesive",
                                                    make_grid_variant_operator<CCMMSHCohesiveTmpl>);

  OperatorNodeFactory::instance()->register_factory("contact_multimat_sphere_hooke_dmt",
