...
ONIKA_HOST_DEVICE_FUNCTION
void cuda_sort_insertion(T* a_ptr, int a_size, Func a_comp)
{
  for(int i = 1; i < a_size ; i++)
  {
    T tmp = a_ptr[i];
    int j = i;
    while( (j > 0) && (comp(tmp,a_ptr[j-1])) // tmp > a_ptr[j-1]
    {
      a_ptr[j] = a_ptr[j-1]      
      j--;
    }
...
ONIKA_HOST_DEVICE_FUNCTION
void cuda_sort_for_friction(size_t* a_ptr, int a_size, GridCellParticleNeigborFriction& a_friction)
{
  for(int i = 1; i < a_size ; i++)
  {
    T tmp = a_ptr[i];
    int j = i;
    while( (j > 0) && ( a_friction.particle_id(tmp)> a_friction.particle_id(a_ptr[j-1]))) // tmp > a_ptr[j-1]
    {
      a_ptr[j] = a_ptr[j-1]      
      j--;
    }
...
#     pragma omp parallel
      {
        ...
        GRID_OMP_FOR_BEGIN(dims-2*gl,_,block_loc, schedule(dynamic) reduction(+:n_missing_particles,n_missing_nbh) )
        {
          ...
          const size_t friction_n_particles = friction.number_of_particles(); // may be 0
          size_t sorted_friction_id [friction_n_particles];
          for(unsigned int i=0;i<friction_n_particles;i++) sorted_friction_id[i]=i;
          cuda_sort_for_friction(sorted_friction_id.data(); sorted_friction_id.size(), friction);
