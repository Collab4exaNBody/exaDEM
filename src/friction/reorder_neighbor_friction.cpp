...
namespace exaDEM
{
  ...
  public:
    ...
    inline void execute () override final
    {
      ...
      if( frictions.m_cell_friction.size() != n_cells )
      {
        ldbg << "number of cells changed, reset friction data" << std::endl;
        frictions.m_cell_friction.clear();
        frictions.m_cell_friction.resize( n_cells );
      }
      assert( frictions.m_cell_friction.size() == n_cells );
...
#     pragma omp parallel
      {
        ...
        GRID_OMP_FOR_BEGIN(dims-2*gl,_,block_loc, schedule(dynamic) reduction(+:n_missing_particles,n_missing_nbh) )
        {
          ...
          auto & friction = frictions.m_cell_friction[cell_a];
          ...
          apply_cell_particle_neighbors(*grid, *chunk_neighbors, cell_a, loc_a, std::false_type() /* not symetric */,
            [cells, cell_a, &shift, &last_p_a, &friction, &reordered_friction, &sorted_friction_id, &sorted_friction_nbh_id, &cur_friction_idx, &n_missing_particles, &n_missing_nbh]
