...
struct UpdateIntertiaSphereFunctor {
  ...
  ONIKA_HOST_DEVICE_FUNC inline void operator()(const double rx, const double ry, const double rz, const uint64_t id,
                                                const double mass, const double radius, Vec3d& inertia) const {
    Vec3d r = {rx, ry, rz};
...
namespace exanb {
template <>
struct ComputeCellParticlesTraits<exaDEM::UpdateIntertiaSphereFunctor> {
...
ONIKA_AUTORUN_INIT(update_inertia) {
  OperatorNodeFactory::instance()->register_factory("update_inertia", make_grid_variant_operator<UpdateInertia>);
}
}  // namespace exaDEM
