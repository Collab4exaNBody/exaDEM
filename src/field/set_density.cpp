...
namespace exaDEM {
template <typename GridT, class = AssertGridHasFields<GridT, field::_radius, field::_mass>>
...
#pragma omp parallel
    {
      GRID_OMP_FOR_BEGIN(dims, i, loc, schedule(dynamic)) {
        ...
        for (size_t j = 0; j < n; j++) {
          ...
          if (m[j] <= 0.0) {
            color_log::error("set_density", "Wrong definition of a mass for the particle " +
                                                std::to_string(cells[i][field::id][j]) + ".");
          }
        }
      }
      GRID_OMP_FOR_END
...
// === register factories ===
ONIKA_AUTORUN_INIT(set_density) {
  OperatorNodeFactory::instance()->register_factory("set_density", make_grid_variant_operator<SetDensity>);
}
