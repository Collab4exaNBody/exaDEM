...
namespace exaDEM
{
   ...
   */
  template <int idx, typename Tuple, typename Arg> ONIKA_HOST_DEVICE_FUNC static inline void setter(const Tuple &values, Arg &arg) { arg = values.get(onika::tuple_index<idx>); }
   ...
   */
  template <int idx, typename Arg, typename Tuple, typename... Args> ONIKA_HOST_DEVICE_FUNC static inline void setter(const Tuple &values, Arg &arg, Args &...args)
  {
   ...
   */
  template <typename... Ts> struct SetFunctor
  {
   ...
   * @tparam Ts Variadic template parameter pack for types.
   */
  template <typename... Ts> struct SetRegionFunctor
  {
     ...
     * @param args Additional arguments to set.
     */
    template <typename... Args> ONIKA_HOST_DEVICE_FUNC inline void operator()(double rx, double ry, double rz, const uint64_t id, Args &...args) const
    {
      Vec3d r = {rx, ry, rz};
   ...
   */
  template <typename Func, typename... Ts> struct SetFunctorWithProcessing
  {
   ...
   * @tparam Ts Variadic template parameter pack for types of default values.
   */
  template <typename Func, typename... Ts> struct SetRegionFunctorWithProcessing
  {
    const ParticleRegionCSGShallowCopy region; /**< Shallow copy of a particle region. */
    mutable Func m_processing;                 /**< Functor with processing. */
    onika::FlatTuple<Ts...> m_default_values;  /**< Flat tuple of default values of types Ts. */
   ...
   */
  template <typename Func> struct GenSetFunctor
  {
    ...
    template <typename... Args> void do_nothing(Args... args) const {}
     ...
     * @param args additional arguments to process.
     */
    template <typename... Args> inline void operator()(Args &...args) const { do_nothing(m_gen(args)...); }
  };
   ...
   * @tparam func the type of the functor with generator.
   */
  template <typename Func> struct GenSetRegionFunctor
  {
    const ParticleRegionCSGShallowCopy region; /**< Shallow copy of a particle region. */
    mutable Func m_gen;                        /**< functor used to generate value. */
     ...
     * @param args additional arguments to process.
     */
    template <typename... Args> inline void operator()(double rx, double ry, double rz, const uint64_t id, Args &...args) const
    {
      Vec3d r = {rx, ry, rz};
      if (region.contains(r, id))
      {
   ...
   * @tparam Ts Variadic template parameter pack for types of default values.
   */
  template <typename... Ts> struct FilteredSetFunctor
  {
     ...
     * @param args Additional arguments to process.
     */
    template <typename... Args> ONIKA_HOST_DEVICE_FUNC inline void operator()(uint32_t type, Args &...args) const
    {
   ...
   * @tparam Ts Variadic template parameter pack for types of default values.
   */
  template <typename... Ts> struct FilteredSetRegionFunctor
  {
    const ParticleRegionCSGShallowCopy region; /**< Shallow copy of a particle region. */
    uint32_t filtered_type;                    /**< The filtered type. */
    onika::FlatTuple<Ts...> m_default_values;  /**< Flat tuple of default values of types Ts. */
     ...
     * @param args Additional arguments to process.
     */
    template <typename... Args> ONIKA_HOST_DEVICE_FUNC inline void operator()(double rx, double ry, double rz, const uint64_t id, uint32_t type, Args &...args) const
    {
      if (type == filtered_type)
      {
        Vec3d r = {rx, ry, rz};
