...
#pragma omp parallel for reduction(+:nvv, nve, nvf, nee, an, anvv, anve, anvf, anee)
			for( size_t i = 0 ; i < cells.size() ; i++ )
			{
				for( auto& item : cells[i].m_data)
				{
					...
					// drivers
					// cylinder
					if( item.type == 4 )  incr_interaction_counters(item, nvc, anvc, an); 
					//surface
					if( item.type == 5 )  incr_interaction_counters(item, nvs, anvs, an); 
					// ball
					if( item.type == 6 )  incr_interaction_counters(item, nvb, anvb, an); 
					//stl
					if( item.type == 7 )  incr_interaction_counters(item, nSvv, anSvv, an); 
					if( item.type == 8 )  incr_interaction_counters(item, nSve, anSve, an); 
					if( item.type == 9 )  incr_interaction_counters(item, nSvf, anSvf, an); 
					if( item.type == 10 ) incr_interaction_counters(item, nSee, anSee, an); 
					if( item.type == 11 ) incr_interaction_counters(item, nSev, anSev, an); 
					if( item.type == 12 ) incr_interaction_counters(item, nSfv, anSfv, an); 
				}
			}
			...
			int idx = 0 ;
			for (auto it : {&nvv, &nve, &nvf, &nee, &nvc, &nvs, &nvb, &nSvv, &nSve, &nSvf, &nSee, &nSev, &nSfv, &an, &anvv, &anve, &anvf, &anee, &anvc, &anvs, &anvb, &anSvv, &anSve, &anSvf, &anSee, &anSev, &anSfv}) *it = val[idx++];
