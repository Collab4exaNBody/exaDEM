...
namespace exaDEM
{
  ...
  template <typename K, typename AnalysisDataPacker2, typename... Args> struct WrapperContactLawForAll2
  {
     ...
     * @param args Additional parameters passed to the kernel function.
     */
    WrapperContactLawForAll2(InteractionSOA2 &d, K &k, AnalysisDataPacker2 &p, Args... args) : data(std::move(d)), kernel(k), packer(p), params(std::tuple<Args...>(args...)) {}
    ...
    template <size_t... Is> ONIKA_HOST_DEVICE_FUNC inline void apply(uint64_t i, tuple_helper::index<Is...> indexes) const
    {
      exaDEM::Interaction item = data(i);
      const auto [dn, pos, fn, ft] = kernel(item, std::get<Is>(params)...);
      //std::tuple<double, Vec3d, Vec3d, Vec3d>
      //double dn = 0;
      //Vec3d pos = {0,0,0};
      //Vec3d fn = {0,0,0};
      //Vec3d ft = {0,0,0};
...
namespace onika
{
  namespace parallel
  {
    ...
    template <typename T, typename K, typename A, typename... Args> struct ParallelForFunctorTraits<exaDEM::WrapperContactLawForAll2<T, K, A, Args...>>
    {
      static inline constexpr bool RequiresBlockSynchronousCall = false;
      static inline constexpr bool CudaCompatible = true;
    };
    ...
    template <typename T, typename K, typename... Args> struct ParallelForFunctorTraits<exaDEM::WrapperForAll2<T, K, Args...>>
    {
      static inline constexpr bool RequiresBlockSynchronousCall = false;
      static inline constexpr bool CudaCompatible = true;
    };
...
namespace exaDEM
{
  ...
  template <typename Kernel, typename... Args> static inline ParallelExecutionWrapper run_contact_law2(ParallelExecutionContext *exec_ctx, int type, Classifier2 &ic, Kernel &kernel, Args &&...args)
  {
    ...
    //WrapperContactLawForAll2 func(ic.waves[type], kernel, packer, args...);
    return parallel_for(size, func, exec_ctx, opts);
    //return parallel_for(ic.waves[type].size(), func, exec_ctx, opts);
  }
} // namespace exaDEM
