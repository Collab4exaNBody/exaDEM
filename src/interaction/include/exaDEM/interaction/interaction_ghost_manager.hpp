...
namespace exaDEM
{
  ...
  struct InteractionGhostManager {
    ...
    void copy_interaction(GridT& grid,
                          GridExtraDynamicStorage& ges) {
        ...
        for(auto& config : recv_cell_config[proc]) {
          ...
          for(size_t i=config.m_shift ; i<config.m_shift+config.m_size ; i++) {
            ...
            // check that the particle is in this cell and update of the particle offset in the cell
            {
              ...
              /* // Precious for debugging								
                 if(!is_idx_exist)
                 {
                 const uint64_t* const __restrict__ ids = cells[owner.cell][field::id];
                 const uint64_t* const __restrict__ old_ids = cells[old][field::id];
                 int rank; MPI_Comm_rank(MPI_COMM_WORLD, &rank);
                 std::cout << "old cell id #" << old << std::endl;
                 ...
                 */
              if(!is_idx_exist) continue; // can append cause not all particles are copied in ghost cells
            }
