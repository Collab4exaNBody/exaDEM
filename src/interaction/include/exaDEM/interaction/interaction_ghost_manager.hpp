...
namespace exaDEM
{
  ...
  struct InteractionGhostManager
  {
    ...
    template<typename InteractionT>
      void setup(exanb::GhostCommunicationScheme& ghost_scheme, MPI_Comm& comm, InteractionT& ges)
      {
        // tags constants
        ...
        for( size_t proc = 0 ; proc < n_procs ; proc++)
        {
          ...
          for (auto &it : partner.m_sends)
          {
            ...
            send_config.push_back({it.m_cell_i, it.m_partner_cell_i,
                static_cast<uint32_t>(n_sent), // use fixed-width if possible
          ...
          if (partner.m_particles_to_send > 0)
          {
            ...
            if (config_size[proc] > 0)
            {
              ...
              MPI_Isend(sbuf[proc].data(),
                  static_cast<int>(sbuf[proc].size() * sizeof(exaDEM::PlaceholderInteraction)),
                  MPI_BYTE,
                  proc,
                  TAG_PAYLOAD,
                  comm,
        ...
        for( size_t proc = 0 ; proc < n_procs ; proc++)
        {
          ...
          if (partner.m_particles_to_receive > 0)
          {
            ...
            if (isize > 0)
            {
              ...
              size_t needed = static_cast<size_t>(last.m_shift) + static_cast<size_t>(last.m_size);
              rbuf[proc].resize(needed);

              MPI_Irecv(rbuf[proc].data(),
                  static_cast<int>(rbuf[proc].size() * sizeof(exaDEM::PlaceholderInteraction)),
                  MPI_BYTE,
                  proc,
                  TAG_PAYLOAD,
                  comm,
        ...
        for(size_t proc=0 ; proc<n_procs ; proc++)
        {
          ...
          for(auto& config : recv_cell_config[proc])
          {
            ...
            auto& storage = ges[config.m_partner_cell_i].m_data;
						if( !grid.is_ghost_cell(config.m_partner_cell_i) ) color_log::error("copy_interaction","This cell is not a ghost");
            ...
            for(size_t i=config.m_shift ; i<config.m_shift+config.m_size ; i++)
            {
              ...
              owner.cell = config.m_partner_cell_i; // defined by another mpi process
              ...
              // check
              {
								...
								{
									color_log::error("copy_interaction", "owner.id: " + std::to_string(owner.id) 
                ...
                color_log::highlight("copy_interaction", "owner.id: " + std::to_string(owner.id)
                      ...
                      + std::to_string(cells[owner.cell][field::id][owner.p]));
							...
							if(found) 
							{
								...
								{
									item.print();
									color_log::error("copy_interaction", "partner.id: " + std::to_string(partner.id) 
