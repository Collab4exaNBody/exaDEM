...
// sequential
inline void extract_history(std::vector<PlaceholderInteraction>& local, const PlaceholderInteraction* const data,
                            const unsigned int size) {
  local.clear();
  for (size_t i = 0; i < size; i++) {
    const auto& item = data[i];
    if (item.pair.type >= InteractionTypeId::NTypes) {
      continue;
    }
    if (item.active()) {
...
bool check_stiked_face(std::vector<exaDEM::PlaceholderInteraction>& interactions, size_t p, ParticleVertexViewT& pvv) {
