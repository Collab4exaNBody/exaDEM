...
namespace exaDEM
{
	struct interaction_manager
	{
		...
		{
			size_t count(0);
			for (auto& it : list) count += it.size();
			return count;
		}

		template<bool use_history>
		void update_extra_storage( CellExtraDynamicDataStorageT<Interaction>& storage )
		{
			...
			for( size_t p = 0 ; p < list.size() ; p++ )
			{
				...
				if constexpr ( use_history ) update_friction_moment(list[p], hist);
