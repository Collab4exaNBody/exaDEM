...
namespace exaDEM
{
  ...
  namespace itools /* interaction tools */
  {
    ...
    {
      InteractionSOA2 m_data; /**< Pointer to the data to be reduced. */
      const FuncT m_func;           /**< Functor that defines how reduction is performed. */
      ResultT *m_reduced_val;       /**< Pointer to the result of the reduction. */
    ...
    struct IOSimInteractionFunctor2
    {
      ...
      ONIKA_HOST_DEVICE_FUNC inline void operator()(IOSimInteractionResult &local, const uint64_t idx,
                                                    // const exaDEM::Interaction* const interactions,
                                                    InteractionSOA2 interactions, reduce_thread_local_t = {}) const
      {
        exaDEM::Interaction I = interactions(idx);
...
#     endif
    ...
    template <typename Func, typename ResultT> static inline ParallelExecutionWrapper reduce_data2(ParallelExecutionContext *exec_ctx, InteractionSOA2 &data, Func &func, uint64_t size, ResultT &result)
    {
//#     if defined(__CUDA_ARCH__) || defined(__HIP_DEVICE_COMPILE__)
#ifdef ONIKA_CUDA_VERSION
        ParallelForOptions opts;
