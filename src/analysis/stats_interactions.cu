...
class StatsInteractions : public OperatorNode {
 ...
 public:
  ...
  inline void execute() final {
    ...
    auto incr_interaction_counters = [null](const PlaceholderInteraction &I,
                                            int &count,
                                            int &active_count,
                                            int &active_global_count,
                                            int &ghost_count,
...
#pragma omp parallel for reduction(+: nvv, nve, nvf, nee, nvvib, \
    ...
    for (size_t i = 0; i < cells.size(); i++) {
      for (auto &item : cells[i].m_data) {
        ...
        // particles
        switch (type) {
          case 0: incr_interaction_counters(item, nvv, anvv, an, gnvv, gn);
                  break;
          case 1: incr_interaction_counters(item, nve, anve, an, gnve, gn);
                  break;
          case 2: incr_interaction_counters(item, nvf, anvf, an, gnvf, gn);
                  break;
          case 3: incr_interaction_counters(item, nee, anee, an, gnee, gn);
                  break;
                  // drivers
                  // cylinder
          case 4: incr_interaction_counters(item, nvc, anvc, an, gnvc, gn);
                  break;
                  // surface
          case 5: incr_interaction_counters(item, nvs, anvs, an, gnvs, gn);
                  break;
                  // ball
          case 6: incr_interaction_counters(item, nvb, anvb, an, gnvb, gn);
                  break;
                  // stl
          case 7: incr_interaction_counters(item, nSvv, anSvv, an, gnSvv, gn);
                  break;
          case 8: incr_interaction_counters(item, nSve, anSve, an, gnSve, gn);
                  break;
          case 9: incr_interaction_counters(item, nSvf, anSvf, an, gnSvf, gn);
                  break;
          case 10: incr_interaction_counters(item, nSee, anSee, an, gnSee, gn);
                   break;
          case 11: incr_interaction_counters(item, nSev, anSev, an, gnSev, gn);
                   break;
          case 12: incr_interaction_counters(item, nSfv, anSfv, an, gnSfv, gn);
                   break;
          case 13: incr_interaction_counters(item, nvvib, anvvib, an, gnvvib, gn);
                   break;
          default: break;
        }
      }
