...
class ParticleCounterAnalysis : public OperatorNode {
  ...
  ADD_SLOT(Traversal, traversal_real, INPUT_OUTPUT, REQUIRED,
 ...
 public:
  ...
  inline void execute() final {
    ...
    if (list_of_types.size() == 0) {
      lout << "[Analysis/counter] types is empty, this operator is skipped" << std::endl;
    }
...
ONIKA_AUTORUN_INIT(counter) {
  OperatorNodeFactory::instance()->register_factory("particle_counter",
