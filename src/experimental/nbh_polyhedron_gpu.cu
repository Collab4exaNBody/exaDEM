...
class UpdateClassifierPolyhedronGPUPCCP : public OperatorNode {
 ...
 public:
  ...
  inline void execute() final {
    ...
    ONIKA_CU_DEVICE_SYNCHRONIZE();
    parallel_for(parallel_range_fpd, func_prefix_driver, parallel_execution_context("nbh_gpu::func_prefix_driver"),
                 opts);

    InteractionHistory history;

    setup_history_clean_ges(cells, cell_ptr, cell_size, *ges, history, st_history);
    ...
    pp_offsets.resize(cell_pair_size);
    {
      void* d_tmp = nullptr;
      size_t tmp_bytes = 0;
      cub::DeviceScan::ExclusiveSum(d_tmp, tmp_bytes, pp_counts.data(), pp_offsets.data(), cell_pair_size);
    ...
    if (total_pp > 0) {
      ...
      for (int t = 0; t < InteractionTypeId::NTypesPP; t++) {
        void* d_tmp = nullptr;
        size_t tmp_bytes = 0;
        cub::DeviceScan::ExclusiveSum(d_tmp, tmp_bytes, type_counts[t].data(), type_prefix[t].data(), total_pp);
        cudaMalloc(&d_tmp, tmp_bytes);
        cub::DeviceScan::ExclusiveSum(d_tmp, tmp_bytes, type_counts[t].data(), type_prefix[t].data(), total_pp);
        cudaFree(d_tmp);
      }
      ...
      PackInteractionPrefix<<<grid_1d, block_1d>>>(interaction_prefix.data(), type_prefix[0].data(),
                                                   type_prefix[1].data(), type_prefix[2].data(), type_prefix[3].data(),
                                                   total_pp);
      ONIKA_CU_DEVICE_SYNCHRONIZE();

      for (int t = 0; t < InteractionTypeId::NTypesPP; t++) {
        total_interactions[t] = type_prefix[t][total_pp - 1] + type_counts[t][total_pp - 1];
      }
    }
    ...
    ClassifyIPDFunc classify_driver = {cells,       cell_driver_accessor, cell_ptr, *rcut_inc,
                                       shps.data(), vertex_fields,        drvs,     classifier_interaction_accessor};
    parallel_for(cell_size, classify_driver, parallel_execution_context("nbh_gpu::classify_driver"), opts);
    ...
    transfer_classifier_grid<do_ghost_only, do_active_interaction_only, false>(
        cell_ptr, info_cell, info_cell_pair, info_cell_driver, classifier_interaction_accessor, *ges,
