...
#include <cub/cub.cuh>
#include <exaDEM/experimental/polyhedron/nbh_gpu/nbh_gpu_pp.hpp>
	...
	inline void execute() final {
		...
		/*{
			...
			for (int typeID = get_first_id<InteractionType::ParticleParticle>();
				...
				for (size_t i = 0; i < (size_t)total_interactions[typeID]; i++) {
					counts[typeID]++;
...
ONIKA_AUTORUN_INIT(nbh_polyhedron_gpu_pccp) {
	OperatorNodeFactory::instance()->register_factory("nbh_polyhedron_gpu_pccp",
