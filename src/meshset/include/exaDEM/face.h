...
//*/
			...
			for (size_t i = 0; i < vertices.size(); ++i) {
				const Vec3d& p1 = vertices[i];
				const Vec3d& p2 = vertices[(i + 1) % vertices.size()];
				Vec3d edge = p2-p1;
				if (exanb::dot(center-p1, edge) >= 0  && exanb::dot(center-p2, edge) <= 0)  {	intersections++;}
				else { break; }
			}

			if(intersections == vertices.size()) 
			{
				Vec3d contact_position = normal*offset; // we need dot(conatct_position, normal)
				return std::make_tuple(true, contact_position, 0);
			}
