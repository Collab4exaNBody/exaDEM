...
namespace exaDEM
{
	...
	*/			
	...
	struct stl_meshes
	{
		...
		onika::memory::CudaMMVector< int > cells_GPU2;
		...
		onika::memory::CudaMMVector< int > cells_active;
		onika::memory::CudaMMVector< int > cells_active2;
		onika::memory::CudaMMVector< int > last_index;
		onika::memory::CudaMMVector< int > first_index;
		onika::memory::CudaMMVector< int > second_index;
		
		onika::memory::CudaMMVector< double > posx;
		onika::memory::CudaMMVector< double > posy;
		onika::memory::CudaMMVector< double > posz;
		onika::memory::CudaMMVector< double > nx_final;
		onika::memory::CudaMMVector< double > ny_final;
		onika::memory::CudaMMVector< double > nz_final;
		
		onika::memory::CudaMMVector< int > compute_cell;
		onika::memory::CudaMMVector< int > compute_face;
		...
		void update_GPU(){
			...
			for(size_t i = 0; i < indexes.size(); i++){
				if(indexes[i].size() > 0){
					...
					if(indexes[i].size() > max_faces) max_faces = indexes[i].size();
				} else {
					nb_faces_GPU.push_back(0);
				}
			}
			
			nb_max_faces= max_faces;
			...
			compute_face.resize(0);
			int nb_vertices=0;
			for(int i = 0; i < nb_max_faces; i++){
				int max_meshes=0;
				for(int j = 0; j < nb_cells; j++){
					if(i < nb_faces_GPU[j]){
						nx_GPU.push_back(nx[indexes[j][i]]);
						ny_GPU.push_back(ny[indexes[j][i]]);
						nz_GPU.push_back(nz[indexes[j][i]]);
						offsets_GPU.push_back(offsets[indexes[j][i]]);
						compute_face.push_back(1);
					} else {
						nx_GPU.push_back(0);
						ny_GPU.push_back(0);
						nz_GPU.push_back(0);
						offsets_GPU.push_back(0);
						compute_face.push_back(0);
					}		 
				}
			}
			...
			for(int i = 0; i < nb_max_faces; i++){
				for(int j = 0; j < 3; j++){
					for(int z = 0; z < nb_cells; z++){
						if(compute_face[z + nb_cells*i]==1){
							vx_GPU.push_back(vx[start[indexes[z][i]]+j]);
							vy_GPU.push_back(vy[start[indexes[z][i]]+j]);
							vz_GPU.push_back(vz[start[indexes[z][i]]+j]);
						} else {
							vx_GPU.push_back(0);
							vy_GPU.push_back(0);
							vz_GPU.push_back(0);
